...
tasks = [
    ...
    ("1.3", "Phase 1: Auth", "Backend", "Build legacy /auth/login bridge for pre-Supabase accounts", "Dev", "Critical", "Done", "Migrates older custom-auth users into Supabase Auth"),
    ("1.4", "Phase 1: Auth", "Backend", "Verify Supabase access tokens on protected routes", "Dev", "High", "Done", "Backend auth middleware no longer issues custom JWTs"),
    ...
    ("1.8", "Phase 1: Auth", "Mobile", "Build RegisterScreen — phone, name, password fields", "Dev", "Critical", "Done", "Direct Supabase sign-up using phone/password UI"),
    ("1.9", "Phase 1: Auth", "Mobile", "Build LoginScreen — phone + password", "Dev", "Critical", "Done", "Direct Supabase login with one-time backend fallback for legacy users"),
...
endpoints = [
    ...
    ("POST",   "/auth/login",                     "Auth",    "Legacy-account bridge into Supabase Auth sessions",     "No",  "1", "Only needed for pre-migration users"),
    ("POST",   "/auth/logout",                    "Auth",    "Revoke current Supabase session from backend if needed","Yes", "1", ""),
    ("POST",   "/auth/refresh",                   "Auth",    "Refresh Supabase session using refresh token",          "No",  "1", ""),
    # Users
...
deps = [
    ...
    ("bcryptjs",                    "^3.x",    "Backend","Legacy-password migration support",      "npm install bcryptjs",     "Used only to migrate older custom-auth users"),
...
integrations = [
    ...
    ("Upstash Redis",      "Cache / KV",     "Optional cache, rate limiting, loadshedding data cache",
     "10k commands/day free",               "Pay-per-use: very cheap",           "1", "Optional",
     "upstash.com — enable only when in-memory fallback is no longer enough"),
