...
tbl(doc,
    ...
    [
        ...
        ['Supabase',  'Latest', 'PostgreSQL DB + Auth + Realtime + Storage', 'Free tier covers MVP — one service handles sessions, data, and live chat'],
        ['Redis',     'Optional', 'Cache and rate limiting', 'Backend now runs without Redis when REDIS_URL is missing'],
        ['Zod',       '4.x',    'Request validation',           'Type-safe validation that matches TypeScript types'],
        ['Supabase Auth', 'Latest', 'Authentication sessions', 'Mobile signs in directly and backend verifies Supabase access tokens'],
        ['Bcrypt',    'Latest', 'Legacy-password migration',   'Used only to migrate older custom-auth users safely'],
...
tbl(doc,
    ...
    [
        ...
        ['Redis / Upstash',       'Optional free tier',                              'Cache and rate limiting when enabled'],
...
tbl(doc,
    ...
    [
        ...
        ['Railway',      'railway.app',           'Optional: create account only when you need hosted backend deployment'],
        ['Upstash Redis','upstash.com',           'Optional: create only if in-memory fallback is no longer enough'],
...
for line in [
    ...
    '',
    '# Redis (optional)',
    'REDIS_URL=your_optional_redis_connection_string',
    '',
    '# Firebase',
...
tbl(doc,
    ...
    [
        ...
        ['src/middleware/',       'auth.middleware.ts',     'Supabase access-token verification — protects all private routes'],
        ...
        ['src/utils/',            'hash.utils.ts',          'Legacy bcrypt password migration helpers'],
...
steps = [
    ('Step 1', 'Project Initialisation', [
        ...
        ('Install backend dependencies',
         'npm install express cors helmet zod bcryptjs @supabase/supabase-js '
    ...
    ('Step 4', 'Authentication', [
        ('What to build',
         'Direct mobile Supabase sign-up/sign-in with a backend legacy-account bridge. '
         'Protected backend routes verify Supabase access tokens instead of issuing custom JWTs.'),
        ('Backend files',
        ...
        ('Build order',
         '1. Disable email confirmation in Supabase Auth for password signup → 2. Build mobile auth screens → 3. Sync app user profiles → 4. Keep backend bridge only for legacy-account migration'),
        ('Test',
         'Register a new user on mobile. Confirm Supabase session exists locally. Use the access token on a protected backend route. Confirm it works.'),
    ]),
...
tbl(doc,
    ...
    [
        ...
        ['Upstash Redis',
         ...
         '1. Create optional instance at upstash.com\n2. Copy REDIS_URL to .env\n3. Use for cache and rate limiting only when needed'],
        ['SA Weather Service',
         'Backend only',
...
tbl(doc,
    ...
    [
        ...
        ['Auth',          'Login existing user',                'Supabase session returned, lands on Chat screen'],
